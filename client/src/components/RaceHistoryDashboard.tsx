...
export type RaceHistoryRace = { raceKey: string; raceDate: string | null; organization: string | null; venue: string | null; raceNo: number | null; raceStatus: string | null; calibrationStatus: string | null; asOf: string | null; resultStatus: string | null; aiPickFinish: number | null; aiPickOutcome: string | null; comparedCount: number | null; exactMatches: number | null; meanAbsoluteRankError: number | null; winReturnRate: number | null; placeReturnRate: number | null; lastSyncedAt: string | null; confirmedAt: string | null };
...
export function RaceHistoryDashboardView({ data, loading = false, refreshing = false, onRefresh, newlyConfirmedRaces = [], onDismissNotifications }: { data: RaceHistoryData | null; loading?: boolean; refreshing?: boolean; onRefresh?: () => void; newlyConfirmedRaces?: RaceHistoryRace[]; onDismissNotifications?: () => void }) {
  ...
  const recent = filteredRaces.slice(-18); const errorState = Boolean(state?.lastError); const isRunning = refreshing || Boolean(state?.syncStartedAt);
...
export function RaceHistoryDashboard() {
  ...
  const refreshing = refresh.isPending || Boolean(dashboard.data?.source?.syncStartedAt);
  ...
  useEffect(() => {
    ...
    if (!races) return;
    const confirmedKeys = races.filter(race => race.resultStatus === "CONFIRMED" && Boolean(race.confirmedAt)).map(race => race.raceKey);
    ...
    if (newlyConfirmed.length) {
      setNewlyConfirmedRaces(newlyConfirmed);
      toast.success("レース結果が確定しました", { description: `新たに${newlyConfirmed.length}件の公式結果を反映しました。` });
    }
    persistConfirmedRaceKeys(confirmedKeys);
  }, [dashboard.data?.races]);
  return <RaceHistoryDashboardView data={dashboard.data ?? null} loading={dashboard.isLoading} refreshing={refreshing} onRefresh={() => refresh.mutate()} newlyConfirmedRaces={newlyConfirmedRaces} onDismissNotifications={() => setNewlyConfirmedRaces([])} />;
}
