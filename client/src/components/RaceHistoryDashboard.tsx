...
export type RaceHistoryRace = { raceKey: string; raceDate: string | null; organization: string | null; venue: string | null; raceNo: number | null; raceStatus: string | null; calibrationStatus: string | null; asOf: string | null; resultStatus: string | null; specialStatuses?: string[] | null; aiPickFinish: number | null; aiPickOutcome: string | null; comparedCount: number | null; exactMatches: number | null; meanAbsoluteRankError: number | null; winReturnRate: number | null; placeReturnRate: number | null; lastSyncedAt: string | null; confirmedAt: string | null; predictionId?: string | null; predictedTop3?: Array<{ mark: "◎" | "○" | "▲"; horseNo: number; horseName: string }>; officialTop3?: Array<{ finish: number; horseNo: number; horseName: string }>; top3Coverage?: number | null };
...
const toRaceHistoryRace = (item: LabResultListItem, requestedDate: string): RaceHistoryRace => ({
  ...
  // Each API entry carries its own saved final_mark -- never derive ◎/○/▲
  // from array position or a rank field (ai_rank/v23k_rank are not marks).
  predictedTop3: item.predicted_top3?.map(horse => ({ mark: horse.mark, horseNo: horse.horse_no, horseName: horse.horse_name ?? `#${horse.horse_no}` })) ?? [],
