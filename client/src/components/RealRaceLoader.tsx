...
export function RealRaceLoader({ onLoad }: { onLoad: (loaded: RealRaceLoad) => void }) {
  ...
  const refresh = () => {
    ...
    fetchRaces(date, org).then((data) => {
      setRaces(data.races);
      setNotice(data.races.length ? `${data.races.length}件のレース候補を取得しました。` : "対象レースがありません。日付または主催を変更してください。");
    }).catch((reason: unknown) => {
      setRaces([]);
      setNotice("");
      setError(`single_pick_aiに接続できません。${reason instanceof Error ? reason.message : String(reason)}`);
  ...
  return <section className="real-race-loader" aria-label="single_pick_ai実レース入力">
    <div className="real-race-heading"><div><span className="eyebrow">REAL RACE INPUT</span><h2>実レースを取り込む。</h2></div><Database size={17} /></div>
