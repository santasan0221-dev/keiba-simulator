...
export function RealRaceLoader({ onLoad }: { onLoad: (loaded: RealRaceLoad) => void }) {
  ...
  return <section className="real-race-loader" aria-label="single_pick_ai実レース入力">
    ...
    <div className={syncStatus.data?.source?.lastError ? "real-race-sync-state is-error" : "real-race-sync-state"}><span>BG SYNC</span><strong>最終成功: {syncStatus.data?.source?.lastSuccessAt ? new Intl.DateTimeFormat("ja-JP", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }).format(new Date(syncStatus.data.source.lastSuccessAt)) : "初回同期前"}</strong><small>次回確認: {syncStatus.data?.source?.nextRetryAt ? new Intl.DateTimeFormat("ja-JP", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }).format(new Date(syncStatus.data.source.nextRetryAt)) : syncStatus.data?.source ? `${syncStatus.data.source.refreshMinutes}分ごと` : "設定を確認中"}</small>{syncStatus.data?.source?.lastError && <p>直近エラー: {syncStatus.data.source.lastError}</p>}</div>
