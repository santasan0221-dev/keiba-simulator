...
export function TruthPanel({ race, loadStatus = "選択日の予測なし" }: { race: LabRace | null; loadStatus?: RealRaceLoadStatus }) {
  ...
  const topPick = calibratedRows[0] ?? null;
  ...
  return <section className={`truth-panel ${ready ? "truth-panel--ready" : "truth-panel--reference"}`} aria-label="single_pick_aiの実AI予測">
    ...
    {ready && topPick ? <div className="truth-panel-summary"><span className="truth-summary-label">AI本命</span><strong>{topPick.name ?? `#${topPick.no}`}</strong><span>勝率 <b>{rawProbability(topPick.model.win_prob_calibrated) ?? "値なし"}</b></span><span>複勝率 <b>{rawProbability(topPick.model.top3_prob) ?? "値なし"}</b></span></div> : <p className="truth-panel-summary truth-panel-summary--withheld">校正済み確率は未表示です（{displayStatus(race.model.calibration_status)}）。0%や推定値へは置き換えません。</p>}
    ...
    {ready ? <><div className="truth-caption"><span>承認済みAI予測</span><small>校正済みの値をAPI応答のまま表示。単位変換・補間は行いません。</small></div><div className="truth-table" role="table" aria-label="校正済みAI予測一覧"><div className="truth-table-head" role="row"><span>AI</span><span>馬名</span><span>win_prob_calibrated</span><span>top3_prob</span></div>{calibratedRows.map((horse, index) => <div className="truth-table-row" role="row" key={`${horse.no}-${horse.name}`}><b>{horse.model.ai_rank ?? index + 1}</b><strong>{horse.name ?? `#${horse.no ?? index + 1}`}</strong><span>{rawProbability(horse.model.win_prob_calibrated) ?? "値なし"}</span><span>{rawProbability(horse.model.top3_prob) ?? "値なし"}</span></div>)}</div></> : <div className="truth-withheld"><strong>校正済み確率は表示しません。</strong><p>このレースは {displayStatus(race.model.calibration_status)} のため、勝率・複勝圏率を推定、補間、またはwhat-ifの数値で代用しません。</p><div>{statuses.map((status) => <code key={status}>{status}</code>)}</div></div>}
