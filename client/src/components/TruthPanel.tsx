...
export function TruthPanel({ race, loadStatus = "選択日の予測なし" }: { race: LabRace | null; loadStatus?: RealRaceLoadStatus }) {
  ...
  return <section className={`truth-panel ${ready ? "truth-panel--ready" : "truth-panel--reference"}`} aria-label="single_pick_aiの実AI予測">
    ...
    {ready ? <><div className="truth-caption"><span>校正済み確率</span><small>API応答のまま表示。単位変換・補間は行いません。</small></div><div className="truth-table" role="table" aria-label="校正済みAI予測一覧"><div className="truth-table-head" role="row"><span>素点順位</span><span>馬名</span><span>win_prob_calibrated</span><span>top3_prob</span></div>{calibratedRows.map((horse) => <div className="truth-table-row" role="row" key={`${horse.no}-${horse.name}`}><b>{horse.model.ai_rank ?? "—"}</b><strong>{horse.name ?? `#${horse.no}`}</strong><span>{rawProbability(horse.model.win_prob_calibrated) ?? "値なし"}</span><span>{rawProbability(horse.model.top3_prob) ?? "値なし"}</span></div>)}</div></> : <div className="truth-withheld"><strong>校正済み確率は表示しません。</strong><p>このレースは{calibrationStatusLabel(race.model.calibration_status)}のため、勝率・複勝圏率を推定、補間、またはwhat-ifの数値で代用しません。</p><div>{statuses.map((status) => <code key={status}>{calibrationStatusLabel(status)}</code>)}</div></div>}
