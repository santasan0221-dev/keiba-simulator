...
export default function BettingCandidatesPage() {
  ...
  return <PublicLabHeader active="betting" eyebrow="FORMAL BETTING / CANONICAL ONLY" title="正式買い目候補" description="校正済み確率と実オッズに基づく正本のBET / NO BET判定だけを表示します。WHAT-IFの数値は使用しません。">
    {result.state === "PENDING_DATA" && !result.data ? <section className="lab-feature-status is-pending" aria-busy="true"><Loader2 size={21} className="spin-icon" /><div><span className="eyebrow">CANONICAL API / VERIFYING</span><h2>正式買い目候補を確認しています。</h2><p>正本の最新予測日とMEMBER閲覧状態を検証するまで、候補・期待値・判定は表示しません。</p></div></section> : <section className={`lab-feature-status ${locked || result.state === "UNAVAILABLE" ? "is-unavailable" : "is-pending"}`} role="status" aria-labelledby="betting-api-title">
      <CircleAlert size={21} aria-hidden="true" />
      <div>
        <span className="eyebrow">FORMAL BETTING / {result.state}</span>
    ...
    {!locked && result.data?.decisions.length ? <section className="lab-candidate-list" aria-label="正本の正式買い目候補">
      {result.data.decisions.map((candidate, index) => <article className="lab-candidate-row" key={`${candidate.raceKey ?? "race"}-${index}`}>
        <div><span className="eyebrow">{candidate.decision}</span><h2>{candidate.raceLabel ?? candidate.raceKey ?? "レース情報を確認中"}</h2><p>{candidate.betType ?? "券種未取得"} · {candidate.horseName ?? "対象馬未取得"}</p></div>
        <dl><div><dt>校正済み確率</dt><dd>{metricText(candidate.calibratedProbability, 3)}</dd></div><div><dt>実オッズ</dt><dd>{metricText(candidate.marketOdds, 3)}</dd></div><div><dt>Expected Return</dt><dd>{metricText(candidate.expectedReturn, 3)}</dd></div><div><dt>Edge</dt><dd>{metricText(candidate.edge, 3)}</dd></div></dl>
    ...
    <section className="lab-principle-grid" aria-label="正式買い目候補の判定経路">
      <div><span>01</span><h2>Calibrated probability</h2><p>TRUTH PANELと同じ正本の校正済み確率が提供された場合だけ使用します。</p></div>
      <div><span>02</span><h2>Real market odds</h2><p>実市場オッズが正本APIから返る場合だけ使用します。推定値は使いません。</p></div>
      <div><span>03</span><h2>Gate decision</h2><p>正本が返すBET / NO BET / UNAVAILABLEをそのまま表示します。自動購入は提供しません。</p></div>
    </section>

    <section className="lab-member-callout">
      <div><LockKeyhole size={18} /><span className="eyebrow">MEMBER CANDIDATE / ENTITLEMENT REQUIRED</span><h2>閲覧権限が正本で確認された場合だけ表示します。</h2><p>対象レース、券種、対象馬、校正済み確率、実オッズ、Expected Return、Edgeは、認証済みの正本応答だけから表示します。</p></div>
